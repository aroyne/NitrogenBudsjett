...
def _add_inflow_to_coastal_waters(results, preloaded_data, current_params, dataset_noise, outflow_tracker):
    ...
    if df_kyst is not None:
        for i in range(len(df_kyst)):
            try:
                year_val = df_kyst.iloc[i, 0]
                if pd.isna(year_val): continue
                year = int(year_val)
                
                if year in EXPECTED_YEARS:
                    collected_years.add(year)
                    # Summerer elvesone, direkte, elvebåret og diffust tilførsel (tonn -> kt)
    ...
    if df_t3_coast is not None and df_t3_source is not None:
        for r in range(1, len(df_t3_coast)):
            try:
                year_val = df_t3_coast.iloc[r, 0]
                if pd.isna(year_val): continue
                year = int(year_val)
                
                if year in EXPECTED_YEARS:
                    collected_years.add(year)
                    # Total tilførsel minus havbruk (index 3 i source-tabellen)
                    val = (float(df_t3_coast.iloc[r, 1]) / 1000.0) - (float(df_t3_source.iloc[r, 3]) / 1000.0)
                    val *= noise_teotil
                    
                    # Korriger med den felles avløpstrømmen
                    ...
                    # Hvis det fantes en tidligere post for dette året (fra tidlig ark), oppdaterer vi verdien
...
def _add_wild_shellfish_and_macroalgae(results, preloaded_data, current_params, dataset_noise):
    """
    Beregner nitrogen ut med villfanget skalldyr og makroalger basert på Fiskeridirektoratets data.
    """
    ...
    if df_fiske_old is not None:
        for r in range(1, len(df_fiske_old)):
            try:
                year_val = df_fiske_old.iloc[r, 0]
                if pd.isna(year_val): continue
                year = int(float(year_val))
...
def _add_surface_water_emissions(results, preloaded_data, current_params, dataset_noise, outflow_tracker):
    ...
    # Modellering ut fra målt retensjon i TEOTIL3
    df_t3_ret = preloaded_data.get('hy_teotil3_retention')
    if df_t3_ret is not None:
        for r in range(1, len(df_t3_ret)):
            try:
                year_val = df_t3_ret.iloc[r, 0]
                if pd.isna(year_val): continue
                year = int(year_val)
    ...
    # Fallback/historisk beregning basert på fast retensjonsbrøk
    for year in sorted(EXPECTED_YEARS):
        if year not in collected_years and outflow_tracker.loc[year, 'entries'] == 1:
            collected_years.add(year)
            ...
            results.append({'flow_name': flow_n2, 'year': year, 'value': max(0.0, hist_ret_val * (1.0 - fraction_N2O)),
                            'comment': 'modeled (Fast retensjonsbrøk påført tilførsel)', 'data_sources': 'Beregningsmodell'})
            results.append({'flow_name': flow_n2o, 'year': year, 'value': max(0.0, hist_ret_val * fraction_N2O),
                            'comment': 'modeled (Fast retensjonsbrøk påført tilførsel)', 'data_sources': 'Beregningsmodell'})

    missing_years = EXPECTED_YEARS - collected_years
    report_missing_years(flow_n2, missing_years, results)
        ...
        for col in range(2, df_art.shape[1]):
            try:
                ...
                if year in EXPECTED_YEARS:
                    collected_years.add(year)
                    val = 0.0
                    # Summerer kommersielle villfisk-grupper fra pelagisk og bunnfisk
...
def _add_aquaculture_internal_flows(results, aquaculture_production_dict, current_params):
    """
    Beregner akvakultur-interne og utgående strømmer (slaktefisk, fekfôrspill, ekskresjon)
    ut fra den felles genererte produksjonsmatrisen.
    """
