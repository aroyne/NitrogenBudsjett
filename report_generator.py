...
def generate_github_pages_report(plot_dir='output_files/plots', output_filename='index.md', bib_filename='library.bib'):
    ...
    # ========================================================
    # 4. NYTT: OPPRETT HIERARKISK PORTAL FOR AGRICULTURE (AG) POOL
    # ========================================================
    ag_folder = "agriculture_pool"
    os.makedirs(ag_folder, exist_ok=True)

    # 4a. Hovedside for Agriculture
    with open(os.path.join(ag_folder, "pool_agriculture.md"), 'w', encoding='utf-8') as f:
        ...
        f.write("* [Manure management, storage and animal husbandry (AG.MM)](subpool_manure_management.html)\n")
        f.write("* [Soil management (AG.SM)](subpool_soil_management.html)\n")

    # 4b. Sub-hovedside for Manure Management (AG.MM)
    ...
    with open(os.path.join(ag_folder, "subpool_soil_management.md"), 'w', encoding='utf-8') as f:
        ...
        f.write("### Flows that are zero or neglected:\n\n")
        f.write("* **AG.SM-FS.FO-Overland flow-Nmix**, **AG.SM-FS.OL-Overland flow-Nmix** and **AG.SM-FS.WL-Overland flow-Nmix** are neglected as suggested by Schäppi (2025) [^schappi_annexes_2025]: *«In a first approximation it can be assumed that N losses to hydrosphere or forests and semi-natural vegetation occur mainly via leaching. If no country specific data is available on fractions for overland flow of N, the overland flows can be neglected for simplification purposes»*.\n")
        f.write("* **AG.SM-PR.SO-Farm crops substrate-Nmix** is farm crops substrate for biofuels production and composting. According to data in SSB table 12359 «Biologisk behandling av avfall, etter materialtype (1 000 tonn) 2017 – 2023» for category «Landbruksavfall, etande”, these values are small enough to be neglected. Since we only have values given for a few years, we have chosen to neglect this flow.\n")
        f.write("* **AG.SM-HY.SW-Overland flow-Nmix** is not included because all runoff and leaching is included in **AG.SM-HY.SW-Leaching-Nmix**.\n")
    ...
    for filename in plot_files:
        ...
        if "ag_mm" in norm:
            ...
            if "application" in norm:
                ...
                description = "Taken from UNFCCC Common reporting tables, Table 3."
            elif "products" in norm and "animal" in norm:
            ...
            elif "nonedible" in norm or ("animal" in norm and "op" in norm):
            ...
            elif "export" in norm and "live" in norm:
        ...
        # MAPPING FOR SOIL MANAGEMENT (AG.SM)
            ...
            elif "emissions" in norm and "n2" in norm and "n2o" not in norm:
            ...
            elif "food" in norm and "crop" in norm:
            ...
            elif "industrial" in norm or ("crop" in norm and "op" in norm):
